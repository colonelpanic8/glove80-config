// ===== GLOVE80 PATCH (shared flash + runtime-config partition access) =====
...
//! subtree). Motivation: on nRF+BLE the only safe flash driver is
//! `nrf_mpsl::Flash` — a singleton that schedules erases/writes into
//! multiprotocol timeslots — and RMK's generated `main` hands it wholesale to
//! the storage task. The Glove80 firmware needs a second flash consumer for
//! its persistent lighting configuration (the reserved runtime-config
//! partition), so the generated code now wraps the singleton in an async
//! mutex and shares it:
//!
...
//! application registers with [`set_window`] (the Glove80 firmware sets its
//! reserved 0xdc000-0xec000 partition), so a bug cannot erase the running
//! image or RMK's storage.
//!
//! Single client by design: exactly one application task may use the
//! [`read`]/[`write`]/[`erase`] helpers (they pair one request with one
//! reply). On the Glove80 that is the central's lighting task.
        ...
        let reply = match REQUESTS.receive().await {
            FlashRequest::Read { addr, len } => {
                ...
                if !in_window(addr, len as u32) {
                    error!("config_flash: read outside window: 0x{:x}+{}", addr, len);
                    FlashReply::Error
                } else {
                    ...
                    match res {
                        ...
                        Err(_) => {
                            error!("config_flash: read failed at 0x{:x}", addr);
                            FlashReply::Error
                        }
                    }
                }
            ...
            FlashRequest::Write { addr, len, data } => {
                let len = (len as usize).min(CHUNK);
                if !in_window(addr, len as u32) {
                    error!("config_flash: write outside window: 0x{:x}+{}", addr, len);
                    FlashReply::Error
                } else {
                    ...
                    match res {
                        Ok(()) => FlashReply::Done,
                        Err(_) => {
                            error!("config_flash: write failed at 0x{:x}", addr);
                            FlashReply::Error
                        }
                    }
                }
            ...
            FlashRequest::Erase { from, to } => {
                if to < from || !in_window(from, to - from) {
                    error!("config_flash: erase outside window: 0x{:x}..0x{:x}", from, to);
                    FlashReply::Error
                } else {
                    ...
                    while at < to {
                        ...
                        if res.is_err() {
                            error!("config_flash: erase failed at 0x{:x}", at);
                            ok = false;
                            break;
                        }
                        at = end;
